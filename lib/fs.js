...
const Files = require('@java/java.nio.file.Files');
...
class Stats {
  constructor (isFile, isDirectory, isBlockDevice, isCharacterDevice, isSymbolicLink, isFIFO, isSocket) {
  ...
  new Promise(function (resolve, reject) {
    try {
      var fIn = new BufferedReader(
        new InputStreamReader(new FileInputStream(location), 'UTF8')
      );
...
exports.rmdirSync = function (path) {
  var f = __cauldron__.getFile(path);
  if (f.isDirectory()) {
