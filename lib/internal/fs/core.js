...
const { async, promisify } = require('internal/cauldron/async');
...
const callbackify = (fn, callback, ...args) => {
  try {
    const result = async(fn.apply(null, args));
...
const existsSync = path => {
  const f = $$cauldron$$.getFile(sanitizePath(path));
  return f.exists();
};
...
const mkdirSync = (path, options) =>
  $$cauldron$$.getFile(sanitizePath(path)).mkdirs();
...
const readdirSync = (path, options) => {
  const directory = $$cauldron$$.getFile(sanitizePath(path));
...
const readFileSync = (path, options) =>
  $$cauldron$$.readFile(sanitizePath(path));
...
const renameSync = (oldPath, newPath) => {
  const oldf = $$cauldron$$.getFile(sanitizePath(oldPath));
  const newf = $$cauldron$$.getFile(sanitizePath(newPath));
