...
const mkdirSync = (path, options) => __cauldron__.getFile(path).mkdirs();
...
const readFileSync = (path, options) => __cauldron__.readFile(path);
...
const realpathSync = (path, options) => _path.resolve(process.cwd(), path);
...
const renameSync = (oldPath, newPath) => {
  const oldf = __cauldron__.getFile(oldPath);
  const newf = __cauldron__.getFile(newPath);
...
const writeSync = (fd, content, offset, length) => {
  if (typeof content === 'string') {
    writeStringToFile(fd, content, offset, 'UTF8'); // TODO: implement length
  } else {
    writeBufferToFile(fd, content, offset, 'UTF8');
  }
};
...
const writeFileSync = (file, data, options) => {
  writeStringToFile(file, data, 0, 'UTF8');
};
