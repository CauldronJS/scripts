...
function resolveFileName (request, parent, isMain) {
  ...
  if (!isMain) {
    if (isRelative) {
      ...
      let directory;
      if (fs.statSync(path.resolve('deps', request)) === 1) {
        directory = 'deps';
      } else {
      ...
      if (pkg) {
        filename = path.resolve(directory, pkg.main || pkg._main);
      }
    }
...
function tryGetJavaObject (request) {
  try {
    ...
    return __cauldron__.evalScript(`Packages.${request}`, 'internal/modules/cjs/loader.js');
  }
}
...
class Module {
  ...
  */
  ...
  static _load (request, parent, isMain) {
    try {
      if (request.startsWith('@java/')) return tryGetJavaObject(request.split('@java/')[1]);
      // TODO: check internal load from non-internal package
      if (NativeModule.exists(request)) return NativeModule.require(request);
      ...
      console.error(`An error occured while loading module ${request}`);
      throw err;
    }
  }
