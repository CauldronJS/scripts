...
This should execute as such:
1. determine the filename the request is targeting
  1. if relative, normalize
  2. if not, determine if the requested module exists in deps/node_modules anywhere
     in the heirarchy
2. if the result is a folder and not a file, read from the `package.json` within the directory
3. return the 'main' field of the package info
4. if none of these exist, return 'index.js'
    ...
    if (isRelative) {
      ...
      if (fs.existsSync(path.resolve('deps', request))) {
        ...
        directory = path.resolve(
          parent.rootDir && parent.rootDir.length > 0 ? parent.rootDir : '',
          'node_modules',
          request
        );
...
function resolveFileName2(request, module, isMain) {
  const ext =
    isMain && !module.parent ? '.js' : module.filename.split('.').slice(-1)[0];
  const root = resolveRequireRoot(
    request,
    path.resolve(isMain ? process.mainModule : module.filename, '..')
  );
  ...
  */
  ...
  static _load(request, parent, isMain) {
    ...
    try {
      ...
      const pkg = mainPackageCache[dirname];
      newModule.config = pkg;
      if (pkg && !pkg.hasRanScript.prestart) {
        const prestartScript = util.get(pkg, 'scripts.prestart');
        if (prestartScript) {
          console.debug(`Running prestart script for ${newModule.id}`);
          try {
            childProcess.spawnSync(prestartScript, {
