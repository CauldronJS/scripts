...
function getLibPackage(request) {
  let stat = fs.statSync(request);
  if (stat === -1) stat = fs.statSync(request + '.js');
  console.log(stat);
  switch (stat) {
    case 0: // file
      break;
...
function resolveRequireRoot(request, currentDir) {
  ...
  if (isRelative) {
    ...
    console.log({ currentDir, request });
    const libPackage = getLibPackage(request);
    if (libPackage) {
      return libPackage;
    }
...
function resolveFileName(request, module, isMain) {
  ...
  console.debug(`Stat of ${request} resulted in ${stat}`);
  switch (stat) {
    case 0: // file
      return root;
    case 1: // directory
    ...
    case -1: // not found
      if (fs.existsSync(`${root}.${ext}`) || fs.existsSync(`${root}.js`)) {
        return `${root}.${ext}`;
      } else {
