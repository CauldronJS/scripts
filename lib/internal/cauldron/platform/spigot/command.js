...
events.on('tabcomplete', event => {
  ...
  let i = 1;
  while (command && i < buffer.length) {
    command = command.subcommands[buffer[i++]];
  }
  if (!command) return;
  const tabComplete = command.tabComplete(event.getSender());
...
class CauldronCommand {
  ...
  register() {
    if (!this.parent && !this.hasBeenRegistered) {
      ...
      console.trace(`Registered command ${this.name}`);
    } else {
  ...
  unregister() {
    if (!this.parent) {
      ...
      console.trace(`Unregistered command ${this.name}`);
