...
const noop = () => {};
if (!globalThis) {
  // global exists but globalThis doesn't
  globalThis.globalThis = global;
}
globalThis.Bukkit = Java.type('org.bukkit.Bukkit');
globalThis.__cauldron__ = Bukkit.getPluginManager().getPlugin('Cauldron');

globalThis.evalScript = (script, filename) =>
  __cauldron__.evalScript(script, filename || 'eval');

globalThis.log = (msg, level = '7', verbose) => {
  if (verbose && !globalThis.debugMessages) return;
...
(function() {
  ...
  process = globalThis.process = NativeModule.require('internal/process/core');
  globalThis.internalBinding = process.binding;
  globalThis.Buffer = NativeModule.require('internal/buffer').Buffer;
  globalThis.debugMessages = process.config.variables.debug_messages;
