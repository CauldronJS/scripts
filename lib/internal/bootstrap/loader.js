...
// weird polyfill that is required by some NodeJS libraries to test for environment
...
function log(level, verbose, content) {
  const sourceMapParser = internalBinding('SourceMapParser');
  const fileReader = internalBinding('FileReader');
  const pathHelpers = internalBinding('PathHelpers');
  const jsUtils = Java.type('com.cauldronjs.utils.JsUtils');
  const Level = Java.type('java.util.logging.Level');
  const JavaException = Java.type('java.lang.Exception');
  const FileNotFoundException = Java.type('java.io.FileNotFoundException');
  ...
  content.forEach((msg) => {
    if (msg === undefined || typeof msg === 'undefined') {
      ...
      //   )
      //   .join('\n');
      $$cauldron$$.log(Level.SEVERE, `\u001b[${level}m${msg.stack}\u001B[0m`);
      ...
      const stack = `${msg.toString()}${[...msg.getStackTrace()]
        ...
        .map((elm) => `\n\t${elm.toString()}`)}`;
      $$cauldron$$.log(Level.SEVERE, `\u001b[${level}m${stack}\u001B[0m`);
    } else {
      $$cauldron$$.log(
        Level.INFO,
...
(function () {
  ...
  function makeGlobal(id) {
    const toImport = NativeModule.require(id);
    Object.defineProperties(
      globalThis,
      Object.getOwnPropertyDescriptors(NativeModule.require(id))
    );
  }
