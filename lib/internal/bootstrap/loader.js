...
const log = (level, verbose, content) => {
  ...
  content.forEach(msg => {
    if (msg === undefined || typeof msg === 'undefined') {
      $$cauldron$$.log(Level.INFO, `\xA7${level}[Cauldron] undefined`);
    } else if (msg === null) {
      $$cauldron$$.log(Level.INFO, `\xA7${level}[Cauldron] null`);
      ...
      $$cauldron$$.log(Level.INFO, `\xA7${level}[Cauldron] ${msg}`);
...
(function() {
  ...
  class NativeModule {
    ...
    compile() {
      const script = Java.type('com.cauldronjs.utils.FileReader').read(
        $$cauldron$$,
        this.filename
      );
      const wrapped = NativeModule.wrap(script);
      const args = buildRequireArgs(this);
      const compiled = $$cauldron$$
        .getMainIsolate()
        .runScript(wrapped, this.filename);
