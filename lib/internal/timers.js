...
function safeRunnable(fn, onError) {
...
function setImmediate(callback, ...args) {
  const task = $$cauldron$$.scheduleTask(safeRunnable(callback), 0);
...
function setInterval(callback, delay, ...args) {
  const task = $$cauldron$$.scheduleRepeatingTask(
    safeRunnable(callback),
    delay,
    delay
  );
...
function setTimeout(callback, delay, ...args) {
  const task = $$cauldron$$.scheduleTask(safeRunnable(callback), delay);
  return new Timeout(callback, null, args, delay, task);
}
